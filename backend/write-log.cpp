...
void make_sure_log_file(void) throws(runtime_error)
{
    if (!log_file.is_open())
    {
        ...
        sprintf(filename, "cpl-log/cpl-back-%li.log", now);
...
void log_write_error_information(string information)
{
    if (information.length())
    {
        ...
        log_file << put_time(&prepared_time, "%Y-%m-%d %H:%M:%S ") << "ERROR" << endl
                 << information << endl;
    }
}
...
void log_write_regular_information(string information)
{
    if (information.length())
    {
        ...
        unique_lock<mutex> guard(log_mutex);
        log_file << put_time(&prepared_time, "%Y-%m-%d %H:%M:%S ") << "INFO" << endl
                 << information << endl;
    }
}
...
ThreadStatCode clear_all_logs(void)
{
    ...
    log_file.close();
    directory_iterator files("cpl-log");
    for (auto &file : files)
        remove(file);
    make_sure_log_file();
    log_write_regular_information("Log file Reset.\n");
