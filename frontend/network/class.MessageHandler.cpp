...
void ClientSocket::MessageHandler::process_received_data(Buffer &recv_buffer)
{
    while (true)
    {
        ...
        memcpy(&payload_len_net, recv_buffer.peek() + 1 + tag_len, sizeof(payload_len_net));
        uint32_t payload_len = ntohl(payload_len_net);

        if (payload_len > Buffer::kMaxFrameSize)
        {
            log_write_error_information("Received frame payload length (" + to_string(payload_len) + ") exceeds limit (" + to_string(Buffer::kMaxFrameSize) + ").");
        ...
        string tag(recv_buffer.peek() + 1, tag_len);
        recv_buffer.retrieve(header_len);
        string payload = recv_buffer.retrieve_as_string(payload_len);

        Handler handler_to_call;
        bool found_handler = false;
        {
            shared_lock lock(handler_rw_mutex_);
        ...
        {
            owner_.thread_pool_.enqueue(0, [h = move(handler_to_call), p = move(payload), tag_copy = tag]() mutable
                                        {
                try {
                    h(p);
                } catch (const exception& e) {
                    log_write_error_information("Handler for tag '" + tag_copy + "' threw an exception: " + string(e.what()));
                } catch (...) {
                    log_write_error_information("Handler for tag '" + tag_copy + "' threw an unknown exception.");
                } });
        }
        else
        {
            log_write_warning_information("No handler found for tag '" + tag + "' and no default handler set. Discarding message payload (size " + to_string(payload.length()) + ").");
        }
    }
}
